...
A condition aware 'Remaining Useful Life' prediction pipeline for
the NASA C-MAPSS dataset

The final model uses an LSTM to estimate capped RUL from the latest 30 operating cycles.

 FD004 is the primary experiment because it contains six operating conditions and two fault modes.

## Problem

There are multiple modules(parts) present in a turbo jet engine, we know that data which is like "used till failure" is not easy to get and definitely not possible in case of jet engines. So the data is used here is generated by a simulator. We have to model the RUL for an engine given it's condition.

#### Insight on how the training data is generated

(One can train a model without understanding/reading this)
...
- Capping the RUL(to 125), We don't want to know exact that the RUL is 204 or 233, they are of same meaning to us. We want to be precise on lower values
- If same engine data's sequences go in both training and validation/testing then it can be considered as leakage(because they can overlap MASSIVELY), therefore we split development data by engine.
...
The tree baselines receive only the final observation. The LSTM receives the complete 30-cycle history. Apart from tree models linear regression was also tested using rolling features.
...
The safety-aware model improved MAE and RMSE and produced a better per-engine C-MAPSS score for 138 of 248 test engines. The baseline LSTM retained a marginal 0.61% advantage in total C-MAPSS score because we used exponential loss(the exponential score is strongly affected by a small number of large errors).
...
The saved inference pipeline includes the fitted setting scaler, KMeans model, condition-specific sensor statistics, feature order and LSTM parameters which we trained using training data.
